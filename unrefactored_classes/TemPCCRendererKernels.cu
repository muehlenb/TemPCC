...
namespace cuda_tempcc {
    ...
    void cudaCopyIntoTraningBuffer(int numBlocks, int blockSize, float4* pointNNTemporalInput, float3* pointNNSpatialInput, float4* pointNNOutput, float4* trainingNNTemporalInput, float3* trainingNNSpatialInput, float4* trainingNNOutput, int* traningShuffledIndices, curandState* randStates,int startOffset, int stride, int validPointsNum){
        cuda_tempcc::copyIntoTraningBuffer<<<numBlocks, blockSize>>>(pointNNTemporalInput, pointNNSpatialInput, pointNNOutput, trainingNNTemporalInput, trainingNNSpatialInput, trainingNNOutput, traningShuffledIndices, randStates, startOffset, stride, validPointsNum);
    }
    ...
    __global__ void densityFiltering(APoint* gpuPoints, int pointsNum, int* occupied, int occupiedSize){
        ...
        int density = 100;
        int sideHalfLength = 250;
        int sideLength = 500;

        if (index < pointsNum){
            int xId = max(min(int(gpuPoints[index].position.x * density + sideHalfLength), sideLength), 0);
            int yId = max(min(int(gpuPoints[index].position.y * density + sideHalfLength), sideLength), 0);
            int zId = max(min(int(gpuPoints[index].position.z * density + sideHalfLength), sideLength), 0);

            int voxelIdx = xId + sideLength * (yId + sideLength * zId);
    ...
    __device__ inline int2 projectPointToCam(float4 point, int camId){
        ...
        if(u >= 0  && v >= 0  && u + 1 < camInfo.lookup3DToImageSize && v + 1 < camInfo.lookup3DToImageSize){
            // Schritt 2: Vier nächstgelegene Punkte bestimmen
            ...
            // Schritt 3: Lineare Interpolation in beiden Dimensionen
            float2 interpTop = lerp(topLeft, topRight, u_offset);
            float2 interpBottom = lerp(bottomLeft, bottomRight, u_offset);

            // Schritt 4: Endgültigen interpolierten Wert berechnen
    ...
    __global__ void copyIntoTraningBuffer(float4* pointNNTemporalInput, float3* pointNNSpatialInput, float4* pointNNOutput, float4* trainingNNTemporalInput, float3* trainingNNSpatialInput, float4* trainingNNOutput, int* traningShuffledIndices, curandState* randStates, int startOffset, int stride, int validPointsNum){
        ...
        for(int camID = 0; camID < ACTIVE_CAMS; ++camID){
            ...
            if(pointID < validPointsNum){
                ...
                for(int i=0; i < NN_TEMPORAL_INPUT_SIZE; ++i){
                    if(i < startTempNum)
                        trainingNNTemporalInput[targetID *  NN_TEMPORAL_INPUT_SIZE + TRAINING_POINT_NUM * NN_TEMPORAL_INPUT_SIZE * camID + i] = float4{0,0,0,1};
                    else
                        trainingNNTemporalInput[targetID *  NN_TEMPORAL_INPUT_SIZE + TRAINING_POINT_NUM * NN_TEMPORAL_INPUT_SIZE * camID + i] = pointNNTemporalInput[originID * NN_TEMPORAL_INPUT_SIZE + validPointsNum * NN_TEMPORAL_INPUT_SIZE * camID + i];
                }

                for(int i=0; i < NN_SPATIAL_INPUT_SIZE; ++i){
                    trainingNNSpatialInput[targetID *  NN_SPATIAL_INPUT_SIZE + TRAINING_POINT_NUM * NN_SPATIAL_INPUT_SIZE * camID + i] = pointNNSpatialInput[originID * NN_SPATIAL_INPUT_SIZE + validPointsNum * NN_SPATIAL_INPUT_SIZE * camID + i];
                }

                trainingNNOutput[targetID + TRAINING_POINT_NUM * camID] = pointNNOutput[originID + validPointsNum * camID];
            }
        }
    }

    /**
         * Processes the main pass. Here:
         *  - the next position of all points is predicted
         *  - some points are merged with cam points.
         *  - some points are marked for removal.
         *
         * @param gpuPoints         Current collection of all points in the scene.
         * @param camOrderedPoints  Points of current frame (in the order of the cameras).
         * @param validPointsNum    Number of valid points, so that [0, validPointsNum) of
         *                          gpuPoints is guaranteed to be valid.
         */
            ...
            for(int camID = 0; camID < ACTIVE_CAMS; ++camID){
                ...
                for(int y=-3; y <= 3; ++y){
                    for(int x=-3; x <= 3; ++x){
                        ...
                        //int pX = max(0, min(639, prj.x + x * spacing + (x < 0 ? -1 : 1) * abs(x * x)));
                        //int pY = max(0, min(575, prj.y + y * spacing + (x < 0 ? -1 : 1) * abs(x * x)));
                        ...
                        //float4 relCamPosWS = {float(pX), float(pY), 0, 0};
                        //float4 relCamPosWS = {camP.position.x, camP.position.y, camP.position.z, 1};
    ...
    __device__ inline void predict(int id, APoint& point, float4* temporalFlows, float4* pointNNOutput, int* shuffledIndices, int validPointsNum, bool applyFlow){
        ...
        //pointNNOutput[id] = MatMultVec4(invCamMatrices[0], {predictedFlowWS.x, predictedFlowWS.y, predictedFlowWS.z, 0});
        //pointNNOutput[id].w = predictedError;
