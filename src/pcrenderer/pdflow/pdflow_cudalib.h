...
public:
    ...
    //Last frames read
    uchar4* color_wf_dev;
    float4* position_wf_dev;

    //Images and coordinates (pointers to pointers...)
    float4* position_old_dev[8];
    float4* position_dev[8];
    float* color_old_dev[8];
    float* color_dev[8];
    ...
    __host__ void readParameters(unsigned int rows_host, unsigned int cols_host, unsigned int height_host, unsigned int width_host,
                                 float lambda_i_host, float lambda_d_host, float mu_host,
                                 float *g_mask, unsigned int levels_host);

    __host__ void copyNewFrames(uchar4 *color_wf, float4 *position_wf, float2* pDF2CS);
    ...
    __host__ void copyMotionField(float4* D);
    ...
    __device__ float interpolatePixelDepth(float4 *mat, float ind_u, float ind_v);
